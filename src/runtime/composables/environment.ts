...
export const useEnvironment = () => {
	// 変数
	const isInitialized = useState<boolean>('ui-environment-isInitialized', () => false) // 初期化フラグ
	const userAgent = useState<string>('ui-environment-userAgent', () => '') // アクセス環境を文字列で保持
	const device = useState<string>('ui-environment-device', () => '')
	const devicePixelRatio = useState<number>('ui-environment-devicePixelRatio', () => 1) // デバイスのピクセル比
	const language = useState<string>('ui-environment-language', () => '')
	const os = useState<{ name: string, version: string }>('ui-environment-os', () => ({ name: '', version: '' }))
	const browser = useState<{ name: string, version: string }>('ui-environment-browser', () => ({ name: '', version: '' }))
	const touchDevice = useState<boolean>('ui-environment-touchDevice', () => false)
